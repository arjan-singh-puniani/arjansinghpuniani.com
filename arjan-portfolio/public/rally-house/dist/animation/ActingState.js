...
const unit = (v) => Number.isFinite(v) ? clamp(v, 0, 1) : .5;
export function deriveActing(state, bond, id = 'mika') {
...
const DEFAULT = { energy: .7, confidence: .6, social: .5, practice: .6, comfort: .6, tone: .6 };
export class ActingState {
    ...
    constructor(id) {
        this.id = id;
        this.values = deriveActing(DEFAULT, undefined, id);
        this.target = { ...this.values };
    }
    setState(state, bond) { this.target = deriveActing(state, bond, this.id); }
    setRole(role) { if (role !== this.role) {
        this.role = role;
        this.roleAge = 0;
    } }
    react(name, duration = 18) { this.reactionName = name; this.reactionAge = 0; this.reactionDuration = clamp(duration, 1, 30); this.reactionWeight = 0; }
    requestMicro(context) { if (this.clock < this.cooldown || this.protected || this.reactionName || this.role === 'speak')
        return; const kind = context === 'notebook' ? 'note-tap' : context === 'wateringCan' ? 'leaf-inspect' : context === 'cup' ? 'cup-turn' : context === 'gear' ? 'string-check' : context === 'warmup' ? 'headband-adjust' : null; if (!kind)
        return; this.micro = kind; this.microUntil = this.clock + 2.8; this.cooldown = this.clock + 24 + (this.id.length % 4) * 5; }
    update(dt, major = false) {
        if (!Number.isFinite(dt) || dt <= 0)
            return;
        this.clock += dt;
        this.roleAge += dt;
        this.protected = major;
        const t = 1 - Math.exp(-dt * 2.8);
        for (const k of Object.keys(this.values))
            this.values[k] += (this.target[k] - this.values[k]) * t;
        if (major || this.clock >= this.microUntil)
            this.micro = null;
        if (this.reactionName) {
            this.reactionAge += dt;
            const age = this.reactionAge - this.values.reactionLatency;
            this.reactionWeight = major ? 0 : smoothstep(0, .45, age) * (1 - smoothstep(this.reactionDuration - 3, this.reactionDuration, age));
            if (age >= this.reactionDuration) {
