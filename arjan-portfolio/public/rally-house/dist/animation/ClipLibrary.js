...
const Z = () => new Vec3();
const empty = () => ({ pelvisY: 0, pelvisZ: 0, hipTwist: 0, torsoTwist: 0, lean: 0, knee: 0, headNod: 0, headTurn: 0, stance: 0, handL: Z(), handR: Z(), footL: Z(), footR: Z() });
const F = (t, p = {}, handL, handR, footL, footR) => ({ t, p, handL, handR, footL, footR });
...
function tupleToVec(v) { return v ? new Vec3(v[0], v[1], v[2]) : new Vec3(); }
function frameSample(k) { const e = empty(); Object.assign(e, k.p ?? {}); e.handL = tupleToVec(k.handL); e.handR = tupleToVec(k.handR); e.footL = tupleToVec(k.footL); e.footR = tupleToVec(k.footR); return e; }
function lerpSample(a, b, t) { const n = (k) => a[k] + (b[k] - a[k]) * t; return { pelvisY: n('pelvisY'), pelvisZ: n('pelvisZ'), hipTwist: n('hipTwist'), torsoTwist: n('torsoTwist'), lean: n('lean'), knee: n('knee'), headNod: n('headNod'), headTurn: n('headTurn'), stance: n('stance'), handL: Vec3.lerp(a.handL, b.handL, t), handR: Vec3.lerp(a.handR, b.handR, t), footL: Vec3.lerp(a.footL, b.footL, t), footR: Vec3.lerp(a.footR, b.footR, t) }; }
export function sampleClip(name, t) {
    ...
    if (t <= frames[0].t)
        return frameSample(frames[0]);
    if (t >= frames[frames.length - 1].t)
        return frameSample(frames[frames.length - 1]);
    for (let i = 0; i < frames.length - 1; i++) {
        const a = frames[i], b = frames[i + 1];
        if (t >= a.t && t <= b.t) {
            const u = smoothstep(0, 1, (t - a.t) / (b.t - a.t));
            return lerpSample(frameSample(a), frameSample(b), u);
        }
    }
    return empty();
}
export function clipForState(state, seed = 0) {
    switch (state) {
        ...
        case 'swingForehand': return 'forehand';
        case 'swingBackhand': return 'backhand';
        case 'serve': return 'serve';
        case 'volley': return 'volley';
        case 'celebrate': return 'celebrate';
        case 'reactMiss': return 'reactMiss';
        case 'coachFeed': return 'coachFeed';
        case 'coachExplain': return 'coachExplain';
