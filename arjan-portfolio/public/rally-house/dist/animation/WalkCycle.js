...
export class WalkCycle {
    ...
    planted = [true, true];
    yaws = [0, 0];
    starts = [new Vec3(), new Vec3()];
    targets = [new Vec3(), new Vec3()];
    active = false;
    swingPhase = [.55, .55];
    reset() { this.active = false; }
    update(position, yaw, distance, stride) {
        const local = (side, forward) => new Vec3(position.x + Math.cos(yaw) * side + Math.sin(yaw) * forward, .11, position.z - Math.sin(yaw) * side + Math.cos(yaw) * forward);
        if (!this.active) {
            this.yaws = [yaw, yaw];
            this.phase = .25;
            this.feet = [local(-.17, .04), local(.17, .04)];
            this.planted = [true, true];
            this.active = true;
        }
        ...
        for (let i = 0; i < 2; i++) {
            ...
            if (stance) {
                if (!this.planted[i]) {
                    this.feet[i] = this.targets[i].clone();
                    landed = true;
                }
                this.planted[i] = true;
            }
                ...
                if (this.planted[i]) {
                    this.starts[i] = this.feet[i].clone();
                    this.swingPhase[i] = phase;
                    this.targets[i] = local(i === 0 ? -.17 : .17, stride * (1 - phase + .275));
                }
                this.planted[i] = false;
                const angle = Math.atan2(Math.sin(yaw - this.yaws[i]), Math.cos(yaw - this.yaws[i]));
                this.yaws[i] += angle * Math.min(1, distance * 24);
                this.targets[i] = local(i === 0 ? -.17 : .17, stride * (1 - phase + .275));
