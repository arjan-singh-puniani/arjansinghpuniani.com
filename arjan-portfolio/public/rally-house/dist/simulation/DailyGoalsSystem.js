...
const rotations = [['greet', 'coach', 'matcha'], ['greet', 'decorate', 'watch'], ['coach', 'matcha', 'watch'], ['greet', 'coach', 'decorate']];
export class DailyGoalsSystem {
    day = 1;
    goals = [];
    claimed = false;
    constructor(day = 1) { this.reset(day); }
    reset(day) { this.day = day; this.claimed = false; this.goals = rotations[(day - 1) % rotations.length].map(kind => ({ kind, ...templates[kind], done: false })); }
    ensureDay(day) { if (day !== this.day)
        this.reset(day); }
    record(kind) { const g = this.goals.find(x => x.kind === kind); if (g)
        g.done = true; }
    get complete() { return this.goals.every(g => g.done); }
    get doneCount() { return this.goals.filter(g => g.done).length; }
    claim() { if (!this.complete || this.claimed)
        return 0; this.claimed = true; return this.goals.reduce((s, g) => s + g.reward, 0); }
    nextLabel() { return this.goals.find(g => !g.done)?.label ?? 'The rest of the day is yours'; }
    serialize() { return { day: this.day, goals: this.goals.map(g => ({ ...g })), claimed: this.claimed }; }
    load(s) { if (!s)
