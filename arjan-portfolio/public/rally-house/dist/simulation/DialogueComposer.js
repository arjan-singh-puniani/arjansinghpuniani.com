...
export class DialogueComposer {
    ...
    compose(c) {
        ...
        if (c.topic === 'challenge' && c.match && ['mika', 'leo'].includes(c.speaker) && c.partner && ['mika', 'leo'].includes(c.partner) && Number.isFinite(c.match.score.mika) && Number.isFinite(c.match.score.leo)) {
            clause = ` The last one was ${c.match.score.mika}–${c.match.score.leo}. I kept the score.`;
            evidence = [c.match.id];
        }
            ...
            clause = ' That last cup was a good idea.';
            evidence = [c.memory.id];
        }
        else if (c.topic === 'company' && c.favoriteId) {
            clause = ' There’s room at the familiar bench.';
            evidence = [c.favoriteId];
        }
        else if (c.topic === 'company' && c.relationship.familiarity >= 35 && c.relationship.warmth >= 40)
            clause = ' No hurry.';
        const key = [c.speaker, c.partner, c.topic, clause].join('|');
        if (this.recent.some(r => r.key === key && c.now - r.at < 360)) {
            clause = '';
            evidence = [];
        }
