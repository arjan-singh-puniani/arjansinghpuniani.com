const beat = (seconds, speaker, line, gesture = 'none', pose = 'talk') => ({ seconds, speaker, line, poses: { [speaker]: pose }, gestures: { [speaker]: gesture } });
...
export class EverydayLife {
    ...
    tick(dt) { if (Number.isFinite(dt) && dt > 0)
        this.time += dt; if (this.time >= this.retentionAt) {
        this.retentionAt = this.time + 60;
        for (const [key, until] of Object.entries(this.cooldowns))
            if (until < this.time && /^(after-|witness-|try-cue-|find-|three-)/.test(key)) {
    ...
    scenes(c) {
        ...
        if (this.pending.includes('bench-reconnect'))
            scenes.push({ id: 'bench-reconnect', title: 'The tactical thoughts can share', people: ['leo', 'mika'], place: 'courtBench', beats: [beat(4, 'leo', 'I left room for your tactical thoughts this time.', 'offer'), beat(4, 'mika', 'They are very compact.', 'nod'), beat(4, 'leo', 'Mine are learning.', 'shrug'), { seconds: 5, poses: { leo: 'watch', mika: 'watch' }, gestures: { mika: 'laugh' } }], memory: 'Leo made room before Lucresia had to ask. They let the joke do the apologizing.' });
        if (this.pending.includes('cold-tea'))
            scenes.push({ id: 'cold-tea', title: 'Barbara rescues Contessa’s tea', people: ['nia', 'coach'], place: 'cafeTable', beats: [{ ...beat(4, 'nia', 'This used to be hot.', 'offer'), prop: 'cup' }, beat(4, 'coach', 'I was only writing one note.', 'think'), beat(4, 'nia', 'You wrote on the back as well.', 'point'), { seconds: 3, poses: { coach: 'drink', nia: 'watch' }, effect: 'cup' }, beat(4, 'coach', 'Thank you for keeping track of the important part.', 'nod')], memory: 'Barbara brought Contessa back to her forgotten tea. Contessa finally put the notes down.' });
        ...
        if (c.weather === 'rain')
            scenes.push({ id: 'rain', title: 'The rain gets a review', people: ['nia', 'leo'], place: 'cafe', beats: [beat(4, 'leo', 'Perfect tennis weather.', 'shrug'), beat(4, 'nia', 'We play indoors.', 'point'), { seconds: 3, poses: { leo: 'watch', nia: 'watch' } }, beat(4, 'leo', 'Exactly.', 'nod'), { seconds: 4, poses: { nia: 'drink', leo: 'watch' } }], memory: 'Leo praised the rain as perfect tennis weather. Barbara reminded him where the roof was.' });
        ...
        for (const scene of scenes) {
            if ((this.counts[scene.id] ?? 0) > 0 && returning[scene.id]) {
                let i = 0;
                for (const b of scene.beats)
                    if (b.line)
                        b.line = returning[scene.id][i++] ?? b.line;
            }
        }
        return scenes.filter(s => s.id !== 'closing' || c.minute >= 1100);
    }
    choose(c, extra = [], rank, allowAfterHours = false) {
        if (this.time < this.next || (!allowAfterHours && (c.minute < 495 || c.minute >= 1280)))
            return null;
        const eligible = [...this.scenes(c), ...extra].filter(s => s.people.every(id => c.free.has(id)) && (this.cooldowns[s.id] ?? 0) <= this.time);
        if (rank)
            return rank(eligible)[0] ?? null;
    ...
    started(scene) { this.next = this.time + 32; return `life-${++this.sequence}`; }
    failed(scene) { this.cooldowns[scene.id] = this.time + 25; this.next = Math.max(this.next, this.time + 6); }
    completed(scene, id, day, matchId, interruptedBy) {
        if (this.history.some(h => h.id === id))
            return false;
        this.counts[scene.id] = (this.counts[scene.id] ?? 0) + 1;
        this.cooldowns[scene.id] = this.time + (scene.minInterval ?? 220);
        this.next = Math.max(this.next, this.time + 12);
        this.pending = this.pending.filter(p => p !== scene.id);
        if (scene.followup && !this.pending.includes(scene.followup))
            this.pending.push(scene.followup);
        if (scene.id === 'score-talk' && matchId)
            this.lastMatch = matchId;
        this.history.unshift({ id, scene: scene.id, title: scene.title, people: [...scene.people], day, at: this.time, memory: scene.memory, interruptedBy, place: scene.place, motif: scene.motif ?? scene.id });
        this.history = this.history.slice(0, 32);
        return true;
    }
    serialize() { return structuredClone({ time: this.time, next: this.next, sequence: this.sequence, cooldowns: this.cooldowns, counts: this.counts, history: this.history, pending: this.pending, lastMatch: this.lastMatch }); }
    load(s) {
        if (!s)
            return;
        const record = (v) => !!v && typeof v === 'object' && !Array.isArray(v) && Object.values(v).every(n => typeof n === 'number' && Number.isFinite(n) && n >= 0);
        if (![s.time, s.next, s.sequence].every(n => Number.isFinite(n) && n >= 0) || !record(s.counts) || !record(s.cooldowns) || !Array.isArray(s.history) || !Array.isArray(s.pending) || !s.pending.every(p => typeof p === 'string') || typeof s.lastMatch !== 'string' || !s.history.every(h => h && typeof h.id === 'string' && typeof h.scene === 'string' && typeof h.title === 'string' && typeof h.memory === 'string' && Number.isFinite(h.day) && Number.isFinite(h.at) && Array.isArray(h.people) && h.people.every(p => ['coach', 'mika', 'leo', 'nia'].includes(p))))
            throw Error('Everyday-life save is invalid. Storage has been left untouched.');
        this.time = s.time;
        this.next = Math.max(s.next, s.time + 5);
        this.sequence = s.sequence;
        this.counts = { ...s.counts };
        this.cooldowns = { ...s.cooldowns };
        this.history = structuredClone(s.history.slice(0, 32));
        this.pending = s.pending.slice(0, 4);
