export class LifeTelemetry {
    ...
    initiators = {};
    log = [];
    sample(dt, people) { this.seconds += dt; this.samples += people.length * dt; this.idle += people.filter(p => p.state === 'idle' && !p.busy).length * dt; this.quiet += people.filter(p => ['watch', 'sit', 'drink', 'stretch'].includes(p.state)).length * dt; }
    completed(scene, day, minute, why, next, favorite = false) {
        const add = (r, k) => r[k] = (r[k] ?? 0) + 1;
        add(this.scenes, scene.family ?? scene.id);
        add(this.places, scene.objectId ?? scene.place);
        add(this.initiators, scene.people[0]);
        if (scene.people.length > 1)
            add(this.pairs, [...scene.people].sort().join('|'));
        this.meaningful++;
        this.rituals += scene.ritual ? 1 : 0;
        this.favoriteUses += favorite ? 1 : 0;
        this.log.push({ at: this.seconds, day, minute, who: [...scene.people], what: scene.title, why, changed: scene.memory, next });
        this.log = this.log.slice(-240);
    }
...
//# sourceMappingURL=LifeTelemetry.js.map