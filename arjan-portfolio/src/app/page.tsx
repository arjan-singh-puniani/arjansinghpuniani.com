...
<div className="v2-photo-caption"><span>Kuzneski Innovation Cup</span><span>Pittsburgh, PA</span></div>
    ...
    <section className="v2-playground-teaser"><div className="shell"><div><p className="v2-section-number">[ 04 — Playground ]</p><h2>Vector Tennis<br /><em>Racket Lab.</em></h2></div><div><p>A physics-inspired interaction where timing, racket state, contact, spin, and trajectory stay visibly connected.</p><div className="mini-causal-chain" aria-label="Input leads to racket state, contact, ball state, trajectory, and feedback">{["Input","Racket state","Contact","Ball state","Trajectory","Feedback"].map((item)=><span key={item}>{item}</span>)}</div><Link href="/playground/vector-tennis">Open the experiment ↗</Link></div></div></section>
