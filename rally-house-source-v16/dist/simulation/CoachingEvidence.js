export const cueFor = (key) => key === 'movement' ? 'recovery' : key === 'crosscourt' ? 'rhythm' : key === 'spacing' ? 'spacing' : 'preparation';
export function summarize(shots) {
    const n = shots.length;
    const avg = (key) => n ? shots.reduce((v, s) => v + s[key], 0) / n : 0;
...
export class CoachingEvidence {
    ...
    cue = 'preparation';
    practicing = false;
    observe(s) { if (s.who !== 'mika' || s.stroke === 'serve')
        return; this.recent.push({ ...s }); this.recent = this.recent.slice(-16); if (this.practicing && this.reps.length < 8)
        this.reps.push({ ...s }); }
    begin(cue) { this.cue = cue; this.before = this.recent.slice(-8); this.reps = []; this.practicing = false; }
