...
export class World {
    ...
    placementMeshes() { const out = []; for (const p of this.placements) {
        out.push({ kind: 'sphere', position: new Vec3(p.x, .025, p.z), scale: new Vec3(p.type === 'bench' ? 1.7 : .75, .035, p.type === 'bench' ? .7 : .65), color: '#4a584e', alpha: .10, unlit: true });
        if (p.type === 'plant') {
            out.push({ kind: 'cone', position: new Vec3(p.x, .3, p.z), scale: new Vec3(.5, .6, .5), color: palette.terracotta }, { kind: 'sphere', position: new Vec3(p.x, 1.0, p.z), scale: new Vec3(.82, .55, .68), color: palette.sage });
        }
        else if (p.type === 'bench') {
            out.push({ kind: 'roundBox', position: new Vec3(p.x, .4, p.z), scale: new Vec3(1.72, .28, .67), color: palette.wood }, { kind: 'roundBox', position: new Vec3(p.x, .77, p.z + .25), scale: new Vec3(1.72, .66, .18), color: palette.woodDark });
        }
        else if (p.type === 'lamp') {
            out.push({ kind: 'cylinder', position: new Vec3(p.x, 1.0, p.z), scale: new Vec3(.06, 2, .06), color: '#5e6b61' }, { kind: 'cone', position: new Vec3(p.x, 2.05, p.z), scale: new Vec3(.66, .56, .66), color: palette.gold, unlit: true });
        }
        else {
            out.push({ kind: 'cylinder', position: new Vec3(p.x, .32, p.z), scale: new Vec3(.64, .55, .64), color: '#88745d' });
            for (let i = 0; i < 6; i++)
                out.push({ kind: 'sphere', position: new Vec3(p.x + ((i % 2) - .5) * .25, .68 + Math.floor(i / 2) * .11, p.z + ((i % 3) - 1) * .12), scale: new Vec3(.16, .16, .16), color: '#d6c35f', unlit: true });
        }
    } return out; }
    canPlace(x, z) { if (!Number.isFinite(x) || !Number.isFinite(z))
        return false; if (x < -12 || x > 12 || z < -9 || z > 9)
        return false; if (x > -3.8 && x < 5.8 && z > -6.6 && z < 6.6)
        return false; if (this.nav.isBlocked(x, z))
        return false; return !this.placements.some(p => Math.hypot(p.x - x, p.z - z) < 1.2); }
}
