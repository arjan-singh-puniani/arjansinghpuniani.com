...
export class Character {
  ...
  private poseFor(state:AnimState,time:number):Pose{
    ...
    if(state==='swingForehand'){const pre=clamp(sp/contact,0,1),post=clamp((sp-contact)/(1-contact),0,1);coil+=(-.62*(1-smoothstep(0,.82,pre))+.66*style.follow*smoothstep(0,1,post))*style.coil;hipCoil+=coil*.52;lean+=.035+.07*Math.sin(sp*Math.PI);knee+=.11*Math.sin(Math.min(1,sp/contact)*Math.PI);weightShift=smoothstep(.18,.75,sp);}
    if(state==='swingBackhand'){const pre=clamp(sp/contact,0,1),post=clamp((sp-contact)/(1-contact),0,1);coil+=(.58*(1-smoothstep(0,.82,pre))-.56*style.follow*smoothstep(0,1,post))*style.coil;hipCoil+=coil*.56;lean+=.04;knee+=.10*Math.sin(Math.min(1,sp/contact)*Math.PI);weightShift=smoothstep(.20,.78,sp);}
