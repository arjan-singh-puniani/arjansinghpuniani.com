...
function nearestAngle(current:number,target:number){let t=target;while(t-current>Math.PI)t-=Math.PI*2;while(t-current<-Math.PI)t+=Math.PI*2;return t;}

export class CameraController {
  target=new Vec3(0,0,0); desiredTarget=this.target.clone(); distance=48; desiredDistance=48;
  azimuth=Math.PI/4; desiredAzimuth=Math.PI/4; elevation=0.62; desiredElevation=.62; position=new Vec3(); fov=26*Math.PI/180; aspect=1;private viewIndex=0;
  ...
  locked=false;
  fitDistance=48; minDistance=16; maxDistance=58;
  private targetVelocity=new Vec3();private distanceVelocity=0;private azimuthVelocity=0;private elevationVelocity=0;
  constructor(private canvas:HTMLCanvasElement){this.setAspect(1.6);this.distance=this.desiredDistance=this.fitDistance;this.update(0);}
   ...
   */
  setAspect(a:number){
    if(Math.abs(a-this.aspect)<1e-4)return;
    this.aspect=a;
  ...
  update(dt:number){
    if(dt<=0){const ce=Math.cos(this.elevation);this.position.set(this.target.x+Math.sin(this.azimuth)*ce*this.distance,this.target.y+Math.sin(this.elevation)*this.distance,this.target.z+Math.cos(this.azimuth)*ce*this.distance);return;}
    ...
    const desiredA=nearestAngle(this.azimuth,this.desiredAzimuth);[this.azimuth,this.azimuthVelocity]=spring(this.azimuth,this.azimuthVelocity,desiredA,7.2,dt);
  ...
  project(v:Vec3){const p=transformPoint(this.viewProjection(),v),r=this.canvas.getBoundingClientRect();return {x:(p.x*.5+.5)*r.width,y:(1-(p.y*.5+.5))*r.height,visible:p.z>-1&&p.z<1};}
}
