...
import type {ObjectAffordances} from './ObjectAffordances.js';

export interface AmbientSocialChoice {host:string;companion:string;object?:Placement;score:number}
...
export class AmbientSocialPlanner {
  choose(ids:string[],placements:Placement[],affordances:ObjectAffordances,relations:RelationshipSystem,busy:Set<string>,recentPairs:string[],absoluteMinute:number):AmbientSocialChoice|null{
    const free=ids.filter(id=>!busy.has(id));
    if(free.length<2)return null;
    let best:AmbientSocialChoice|null=null;
    for(const host of free){
      ...
      for(const companion of free){
        ...
        if(!best||score>best.score)best={host,companion,object,score};
      }
    }
    return best;
  }
  private jitter(s:string){let h=2166136261;for(let i=0;i<s.length;i++){h^=s.charCodeAt(i);h=Math.imul(h,16777619)}return ((h>>>0)%1000)/1000;}
}
