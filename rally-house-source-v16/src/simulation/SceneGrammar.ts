import type {LifeScene,LifeBeat,MemberId} from './EverydayLife.js';
import {CharacterMind,MEMBERS,type MindContext,type Motive} from './CharacterMind.js';
import type {Placement} from '../world/World.js';

export interface SceneContext extends MindContext {objects:Placement[];usualKnown:boolean;courtActive?:boolean;}
const N:Record<MemberId,string>={coach:'Contessa',mika:'Lucresia',leo:'Leo',nia:'Barbara'};
const line=(speaker:MemberId,text:string,gesture:'none'|'shrug'|'point'|'laugh'|'think'|'offer'|'inspect'|'nod'='none',seconds=4):LifeBeat=>({seconds,speaker,line:text,poses:{[speaker]:'talk'},gestures:{[speaker]:gesture}});
const quiet=(id:MemberId,pose:'watch'|'stretch'|'shuffle'|'drink'='watch',seconds=7,prop?:LifeBeat['prop']):LifeBeat=>({seconds,poses:{[id]:pose},gestures:{[id]:pose==='watch'?'inspect':'none'},prop});
const make=(id:string,title:string,people:MemberId[],place:string,motive:Motive,beats:LifeBeat[],memory:string,extra:Partial<LifeScene>={}):LifeScene=>({id,title,people,place,motive,beats,memory,family:motive,motif:id,minInterval:180,...extra});

/** Systemic setup; small authored reactions; consequences remain in completion. */
export class SceneGrammar {
 compose(mind:CharacterMind,c:SceneContext):LifeScene[]{
  const out:LifeScene[]=[];
  const favorite=(id:MemberId,type?:Placement['type'])=>c.objects.filter(p=>(!type||p.type===type)&&c.favorite(id,p.id))[0];
  for(const id of MEMBERS){
   ...
   const restText:Record<MemberId,string>={coach:'The note can wait for the tea this time.',mika:'Just a minute. Then I’ll know what I want to try.',leo:'This is recovery. I am being very disciplined.',nia:'The club can manage while I finish this.'};
   ...
   if(c.courtActive&&(id==='coach'||id==='nia'))out.push(make(`watch-court-${id}`,`${who} watches without interrupting`,[id],'courtBench','witness',[{seconds:8,poses:{[id]:'watch'},gestures:{[id]:'none'}},{seconds:8,poses:{[id]:'watch'},gestures:{[id]:'nod'}}],`${who} made time to watch the game.`,{family:'watch-court',motif:'pay-attention',minInterval:200,ritual:`watch-${id}`}));
   if(id==='mika'||id==='leo'){
   ...
   if(id==='coach')out.push(make('june-quiet-notes','Contessa leaves a little space on the page',[id],'cafeTable','reflect',[quiet(id,'watch',9,'notebook'),line(id,'One useful sentence. The rest can wait.','think'),quiet(id,'watch',8,'notebook')],'Contessa kept her notes short and left room to look up.',{ritual:'june-notes',motif:'quiet-notes'}));
   if(id==='leo')out.push(make('leo-strings','Leo checks what he already knows',[id],'stringing','gear',[quiet(id,'watch',7),line(id,'Even tension. No mysterious new problem.','inspect'),quiet(id,'watch',6)],'Leo checked his strings and kept the racket he already knew.',{ritual:'leo-strings',motif:'equipment-check'}));
   if(id==='nia'){
  ...
  const invitation:Record<MemberId,string>={coach:'A short break? I could use another pair of eyes on the day.',mika:'Do you have a minute? I’m trying not to spend all of mine practicing.',leo:'I was about to take a very tactical break.',nia:'There’s room for one more. No need to make an occasion of it.'};
  const reply:Record<MemberId,string>={coach:'A minute without a correction. That sounds useful.',mika:'Yes. I can leave one thing unfinished.',leo:'I can spare a minute. Several, if they’re good.',nia:'I was hoping someone would ask me for once.'};
  for(const actor of MEMBERS)for(const partner of MEMBERS){if(actor===partner)continue;
  ...
  for(const e of mind.events.slice(0,12)){
   ...
   for(const actor of e.pending){
    ...
    if(e.kind==='match'&&['mika','leo'].includes(actor)){
     ...
     const loser=e.winner==='mika'?'leo':'mika',subject:MemberId=e.kind==='match'&&actor==='nia'&&e.winner!=='draw'&&mind.states[loser].tone<.58?loser:e.subject,isWin=e.kind==='match'&&e.winner===subject,bench=favorite(subject,'bench');
