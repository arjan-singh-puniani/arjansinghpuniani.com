...
export interface MemberBook {mood?:string;intention?:string;favorite?:string;personalMemory?:string;possessions?:string[];name:string;role:string;tier:string;familiarity:number;warmth:number;trust:number;goal:string;quirk:string;memories:string[]}
export interface BookData {culture?:string;philosophy?:string;
  mikaProgress:number;equipment:{frame:string;tension:number;string:string};coins:number;day:number;time:string;weather:string;
  heart:number;coachLevel:string;coachXP:number;bestRally:number;placements:number;goals:DailyGoal[];moments:string[];members:MemberBook[];
}
...
export class HUD {
  ...
  update(stats:UIStats,paused:boolean,speed:number){byId('coins').textContent=String(Math.floor(stats.coins));byId('stars').textContent=String(stats.stars);byId('heart').textContent=String(stats.heart);byId('clock').textContent=stats.clock;byId('weatherChip').firstChild!.textContent=stats.weather==='rain'?'☂ ':stats.weather==='cloudy'?'☁ ':'☀ ';byId<HTMLButtonElement>('weatherBtn').textContent=stats.weather==='rain'?'☂':stats.weather==='cloudy'?'☁':'☀';byId<HTMLButtonElement>('playBtn').textContent=paused?'▶':'Ⅱ';byId('playBtn').setAttribute('aria-label',paused?'Resume':'Pause');byId<HTMLButtonElement>('playBtn').classList.toggle('primary',!paused);byId<HTMLButtonElement>('speedBtn').textContent=`${speed}×`;}
  setObjective(title:string,text:string){byId('objectiveTitle').textContent=title;byId('objectiveText').textContent=text;}
  showContext(eyebrow:string,title:string,text:string,meta:string[],actions:ActionDef[]){byId('contextEyebrow').textContent=eyebrow.toUpperCase();byId('contextTitle').textContent=title;byId('contextText').textContent=text;const mb=byId('contextMeta');mb.innerHTML='';for(const x of meta){const s=document.createElement('span');s.textContent=x;mb.appendChild(s)}const box=byId('contextActions');box.innerHTML='';for(const a of actions){const b=document.createElement('button');const strong=document.createElement('strong');strong.textContent=a.title;const span=document.createElement('span');span.textContent=a.subtitle;b.append(strong,span);b.onclick=()=>a.run();box.appendChild(b)}this.toggleBuild(false);this.context.scrollTop=0;this.book.classList.remove('open');this.context.classList.add('open');this.focusPanel(this.context);}
  closeContext(){const had=this.context.contains(document.activeElement);this.context.classList.remove('open');if(had)this.opener?.focus();}
  ...
  positionSpeech(x:number,y:number){const el=byId('speech');const half=el.getBoundingClientRect().width/2;el.style.left=`${Math.max(half+12,Math.min(innerWidth-half-12,x))}px`;el.style.top=`${Math.max(145,Math.min(innerHeight-100,y))}px`;}
}
